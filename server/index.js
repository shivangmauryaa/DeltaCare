...
const sendAutomaticEventEmail = async ({ entityType, entityId, event, actorId = 'SYSTEM', extraBody = '', recipientId = null }) => {
  ...
  if (!eventConfig) return { status: 'skipped', detail: 'No automatic email is configured for this event.' };
  ...
  let delivery;
  if (!entity) delivery = { status: 'unavailable', detail: 'The record no longer exists.', attempts: 0 };
  else if (!recipient) delivery = { status: 'unavailable', detail: 'The record does not have a contactable reporter.', attempts: 0 };
  else if (recipient.preferences?.email === false) delivery = { status: 'opted_out', detail: `${recipient.name} has disabled email notifications.`, attempts: 0 };
  else delivery = await sendEmailMessage({ to: recipient.email, subject, body, actionPath, deliveryId });
  ...
  const communication = await mutateDb((latest) => { const record = { id: deliveryId, entityType, entityId, recipientId: recipient?.id || entity?.reporterId || null, templateKeys: [eventConfig.templateKey], customMessage: extraBody, channels: ['email'], deliveries: [{ channel: 'email', ...delivery }], subject, body, automatic: true, event, sentBy: actorId, createdAt: now() }; latest.communications.unshift(record); addAudit(latest, 'email.automatic_delivery', actorId, { communicationId: record.id, event, entityType, entityId, status: delivery.status, attempts: delivery.attempts || 0 }); return record; });
  return { ...delivery, communicationId: communication.id };
};
