...
import { verifySignature as vonageVerifySignature } from '@vonage/jwt';
import crypto from 'node:crypto';
import os from 'node:os';
import fs from 'node:fs/promises';
import path from 'node:path';
import { fileURLToPath } from 'node:url';
import { articleDefaults, registerAdvancedFeatures } from './advancedFeatures.js';
...
app.use((req, res, next) => { res.setHeader('X-Content-Type-Options', 'nosniff'); res.setHeader('X-Frame-Options', 'DENY'); res.setHeader('Referrer-Policy', 'strict-origin-when-cross-origin'); res.setHeader('Permissions-Policy', 'camera=(self), geolocation=(self), microphone=()'); res.setHeader('Cross-Origin-Opener-Policy', 'same-origin'); req.requestId = crypto.randomUUID(); res.setHeader('X-Request-ID', req.requestId); next(); });
...
const publicUser = ({ passwordHash, security, ...user }) => ({ ...user, mfaEnabled: Boolean(security?.mfaEnabled) });
...
const ensureCollections = (db) => {
  const arrays = ['users', 'sessions', 'passwordResets', 'authChallenges', 'issues', 'lostFound', 'auditLogs', 'notifications', 'comments', 'claims', 'matches', 'custodyEvents', 'jobs', 'savedViews', 'backups', 'escalations', 'moderationReports', 'feedback', 'communications', 'messages', 'twilioMessages', 'vonageMessages', 'pushSubscriptions', 'pickups', 'reminders', 'knowledgeBase', 'securityEvents'];
  ...
  if (!Array.isArray(s.vonageAllowedRecipients) || !s.vonageAllowedRecipients.length) s.vonageAllowedRecipients = (process.env.VONAGE_ALLOWED_RECIPIENTS || '').split(',').map((x) => x.trim()).filter(Boolean);
  if (!Array.isArray(s.pickupLocations) || !s.pickupLocations.length) s.pickupLocations = ['Student Services desk', 'Main library help desk', 'Campus security office'];
  if (!s.reminderRules) s.reminderRules = { issueDueSoonHours: 4, pickupReminderHours: 24, enabled: true };
  if (!db.knowledgeBase.length) db.knowledgeBase = articleDefaults.map((article) => ({ ...article, published: true, createdBy: 'SYSTEM', createdAt: now() }));
...
const cleanAttachments = (attachments) => (Array.isArray(attachments) ? attachments : []).slice(0, 5).flatMap((file) => {
  const type = String(file.type || ''); const data = String(file.data || ''); if (!['image/jpeg', 'image/png', 'image/webp'].includes(type) || Number(file.size) > 2 * 1024 * 1024 || !data.startsWith(`data:${type};base64,`)) return [];
  try { const buffer = Buffer.from(data.split(',')[1] || '', 'base64'); const signatures = { 'image/jpeg': buffer[0] === 0xff && buffer[1] === 0xd8 && buffer[2] === 0xff, 'image/png': buffer.slice(0, 8).equals(Buffer.from([0x89, 0x50, 0x4e, 0x47, 0x0d, 0x0a, 0x1a, 0x0a])), 'image/webp': buffer.slice(0, 4).toString() === 'RIFF' && buffer.slice(8, 12).toString() === 'WEBP' }; if (buffer.length > 2 * 1024 * 1024 || !signatures[type] || buffer.toString('latin1').includes('EICAR-STANDARD-ANTIVIRUS-TEST-FILE')) return []; return [{ id: makeId('ATT'), name: path.basename(String(file.name || 'image')).replace(/[^a-zA-Z0-9._ -]/g, '_'), type, size: buffer.length, sha256: crypto.createHash('sha256').update(buffer).digest('hex'), data, scan: { status: 'clean', engine: process.env.CLAMAV_HOST ? 'clamav-adapter' : 'built-in-signature', scannedAt: now() }, storage: { provider: process.env.UPLOAD_PROVIDER || 'inline' }, ocr: { status: 'queued', text: '' } }]; } catch { return []; }
});
...
const VONAGE_TEXT_MAX = 4096;
const vonageHost = (url, fallback) => String(url || fallback).replace(/\/v1\/messages$/, '');
let cachedVonageClient = null;
const vonageClient = async () => {
  if (cachedVonageClient) return cachedVonageClient;
  if (process.env.VONAGE_MOCK === '1') { cachedVonageClient = { messages: { send: async (params) => ({ messageUUID: `MOCK-${crypto.randomBytes(8).toString('hex')}`, params }) } }; return cachedVonageClient; }
  const applicationId = process.env.VONAGE_APPLICATION_ID; const privateKeyPath = process.env.VONAGE_PRIVATE_KEY_PATH;
  if (!applicationId || !privateKeyPath) return null;
  const privateKey = await fs.readFile(privateKeyPath, 'utf8');
  cachedVonageClient = new Vonage({ applicationId, privateKey }, { apiHost: vonageHost(process.env.VONAGE_MESSAGES_API_URL, 'https://messages-sandbox.nexmo.com/v1/messages') });
  return cachedVonageClient;
};
const vonageE164 = (value) => { const normalized = normalizePhone(value); return normalized ? normalized.replace(/^\+/, '') : null; };
const vonageWindowOpen = (db, to) => { const since = Date.now() - 24 * 60 * 60 * 1000; return db.vonageMessages.some((m) => m.channel === 'whatsapp' && m.direction === 'inbound' && String(m.from || '').replace(/^\+/, '') === to && new Date(m.timestamp || m.createdAt).getTime() >= since); };
const isVonageAllowedRecipient = (db, to) => (db.settings?.vonageAllowedRecipients || []).includes(to);
const vonageSignature = (req, res, next) => { const secret = process.env.VONAGE_SIGNATURE_SECRET; if (!secret) return res.status(503).json({ message: 'Vonage webhooks are not configured (VONAGE_SIGNATURE_SECRET is missing from .env).' }); const header = req.headers.authorization || ''; const token = header.startsWith('Bearer ') ? header.slice(7).trim() : header.trim(); if (!token) return res.status(401).json({ message: 'Missing Vonage webhook signature.' }); let valid = false; try { valid = vonageVerifySignature(token, secret); } catch { valid = false; } if (!valid) return res.status(403).json({ message: 'Invalid Vonage webhook signature.' }); const claims = jwt.decode(token) || {}; const rawBody = req.rawBody ? req.rawBody.toString('utf8') : JSON.stringify(req.body); const hash = crypto.createHash('sha256').update(rawBody).digest('hex'); if (claims.payload_hash && claims.payload_hash !== hash) return res.status(403).json({ message: 'Vonage webhook payload hash mismatch.' }); req.vonageClaims = claims; next(); };
...
app.post('/api/auth/login', async (req, res) => {
  ...
  if (user.security?.mfaEnabled) {
    const challenge = { id: makeId('MFA'), userId: user.id, used: false, createdAt: now(), expiresAt: new Date(Date.now() + 5 * 60000).toISOString() };
    await mutateDb((latest) => { latest.authChallenges = latest.authChallenges.filter((item) => item.expiresAt > now() && !item.used); latest.authChallenges.push(challenge); addAudit(latest, 'mfa.challenge_created', user.id); });
    return res.json({ mfaRequired: true, challengeId: challenge.id, message: 'Enter the six-digit code from your authenticator app.' });
  }
...
app.post('/api/issues', auth, async (req, res) => {
  const { title, description, category, location, priority = 'medium', coordinates = null, attachments = [], impact = 'medium', urgency = 'medium', anonymous = false, buildingId, buildingName, floor, floorName, roomId, roomName, spaceType, assetName } = req.body;
  ...
  const issue = await mutateDb((db) => {
    ...
    const item = { id: makeId('ISS'), version: 1, reporterId: anonymous ? 'ANONYMOUS' : req.user.id, anonymous: Boolean(anonymous), title: title.trim(), description: description.trim(), category: ai.category, location: location.trim(), coordinates, attachments: cleanAttachments(attachments), priority: ai.priority, impact, urgency, status: 'submitted', departmentId: dept?.id, assigneeId: null, buildingId: buildingId || null, buildingName: buildingName || null, floor: floor || null, floorName: floorName || null, roomId: roomId || null, roomName: roomName || null, spaceType: spaceType || null, assetName: assetName || null, ai: { category: ai.category, priority: ai.priority, matrixPriority, confidence: ai.confidence, factors: ai.factors, safetyRuleApplied: ai.safetyRuleApplied }, duplicateCandidates: duplicates, parentIssueId: null, sla: { policy: `${ai.priority} priority`, startedAt: createdAt, dueAt: businessDue(db, createdAt, slaHours), breached: false, paused: false }, statusHistory: [{ status: 'submitted', actorId: anonymous ? 'ANONYMOUS' : req.user.id, at: createdAt }], feedback: null, createdAt, updatedAt: createdAt };
...
app.post('/api/lost-found', auth, async (req, res) => {
  ...
  const emailTasks = [];
  if (record.kind === 'found') emailTasks.push(sendAutomaticEventEmail({ entityType: 'lostFound', entityId: record.id, event: 'found_item_logged', actorId: req.user.id }));
  const latest = await readDb(); const matchedLostIds = [...new Set(latest.matches.filter((match) => match.sourceItemId === record.id).map((match) => [record.id, match.candidateItemId].map((itemId) => latest.lostFound.find((item) => item.id === itemId)).find((item) => item?.kind === 'lost')?.id).filter(Boolean))];
  matchedLostIds.forEach((lostId) => emailTasks.push(sendAutomaticEventEmail({ entityType: 'lostFound', entityId: lostId, event: 'lost_match', actorId: 'SYSTEM', extraBody: 'A newly submitted report may match your lost item. Sign in to review the match information and contact Campus Lost & Found.' })));
  const emailDeliveries = (await Promise.allSettled(emailTasks)).map((result) => result.status === 'fulfilled' ? result.value : { status: 'failed', detail: result.reason?.message || 'Automatic email failed.' });
  res.status(201).json({ record, emailDeliveries });
});
...
app.patch('/api/admin/issues/:id', auth, adminOnly, async (req, res) => {
  ...
  try {
    const transition = await mutateDb((db) => {
      ...
      return { issue: item, previousStatus };
    });
    const event = transition.previousStatus !== transition.issue.status && transition.issue.status === 'resolved' ? 'issue_resolved' : transition.previousStatus !== transition.issue.status && transition.issue.status === 'closed' ? 'issue_closed' : null;
    const emailDelivery = event ? await sendAutomaticEventEmail({ entityType: 'issue', entityId: transition.issue.id, event, actorId: req.user.id }) : null;
    res.json({ issue: transition.issue, emailDelivery });
...
app.patch('/api/admin/lost-found/:id', auth, adminOnly, async (req, res) => {
  ...
  try {
    const transition = await mutateDb((db) => {
      ...
      return { record: item, previousStatus };
    });
    const changed = transition.previousStatus !== transition.record.status; const event = changed && transition.record.status === 'possible_match' && transition.record.kind === 'lost' ? 'lost_match' : changed && transition.record.status === 'returned' ? 'item_returned' : null;
    const emailDelivery = event ? await sendAutomaticEventEmail({ entityType: 'lostFound', entityId: transition.record.id, event, actorId: req.user.id }) : null;
    res.json({ record: transition.record, emailDelivery });
  } catch (error) { res.status(404).json({ message: error.message }); }
});
...
app.patch('/api/admin/claims/:id', auth, adminOnly, async (req, res) => {
  if (!['approved', 'rejected'].includes(req.body.status)) return res.status(400).json({ message: 'Invalid claim decision.' });
  try {
    const result = await mutateDb((db) => {
      const item = db.claims.find((entry) => entry.id === req.params.id);
      if (!item) throw new Error('Claim not found.');
      item.status = req.body.status; item.decisionReason = String(req.body.reason || '').trim(); item.decidedBy = req.user.id; item.decidedAt = now();
      const lostItem = db.lostFound.find((entry) => entry.id === item.itemId);
      if (lostItem && item.status === 'approved') { lostItem.status = 'returned'; lostItem.updatedAt = now(); db.custodyEvents.push({ id: makeId('CST'), itemId: lostItem.id, action: 'checked_out', from: lostItem.inventory?.storageBin || 'INTAKE', to: 'Claimant', actorId: req.user.id, recipientId: item.claimantId, createdAt: now() }); }
      notify(db, item.claimantId, `Claim ${item.status}`, `Your ownership claim has been ${item.status}.`, 'lostFound', item.itemId); addAudit(db, `claim.${item.status}`, req.user.id, { claimId: item.id, reason: item.decisionReason });
      return { claim: item, record: lostItem };
    });
    const recipientIds = result.claim.status === 'approved' ? [...new Set([result.record?.reporterId, result.claim.claimantId].filter(Boolean))] : [];
    const emailDeliveries = (await Promise.allSettled(recipientIds.map((recipientId) => sendAutomaticEventEmail({ entityType: 'lostFound', entityId: result.claim.itemId, event: 'item_returned', actorId: req.user.id, recipientId, extraBody: recipientId === result.claim.claimantId ? 'Your ownership claim was approved. Please follow the Campus Lost & Found handover instructions.' : 'The approved ownership claim has completed this item report.' })))).map((delivery) => delivery.status === 'fulfilled' ? delivery.value : { status: 'failed', detail: delivery.reason?.message || 'Automatic email failed.' });
    res.json({ claim: result.claim, emailDeliveries });
  } catch (error) { res.status(404).json({ message: error.message }); }
});
...
app.post('/api/admin/matches/:id', auth, requirePerm('matches:review'), async (req, res) => {
  if (!['accepted', 'rejected'].includes(req.body.status)) return res.status(400).json({ message: 'Invalid match decision.' });
  try {
    const result = await mutateDb((db) => {
      const item = db.matches.find((entry) => entry.id === req.params.id);
      if (!item) throw new Error('Match not found.');
      item.status = req.body.status; item.reviewedBy = req.user.id; item.reviewedAt = now(); item.reviewNote = String(req.body.reason || '').trim();
      const related = [item.lostItemId, item.foundItemId, item.sourceItemId, item.candidateItemId].filter(Boolean).map((id) => db.lostFound.find((record) => record.id === id)).filter(Boolean); const lost = related.find((record) => record.kind === 'lost'); const found = related.find((record) => record.kind === 'found');
      if (item.status === 'accepted') { [lost, found].filter(Boolean).forEach((record) => { record.status = 'possible_match'; record.updatedAt = now(); }); if (lost) notify(db, lost.reporterId, 'Possible match for your lost item', `We found a possible match for ${lost.itemName}. Open DeltaCare to review it.`, 'lostFound', lost.id); }
      addAudit(db, `match.${item.status}`, req.user.id, { matchId: item.id, lostItemId: lost?.id || item.lostItemId, foundItemId: found?.id || item.foundItemId });
      return { match: item, lost, found };
    });
    const emailDelivery = result.match.status === 'accepted' && result.lost ? await sendAutomaticEventEmail({ entityType: 'lostFound', entityId: result.lost.id, event: 'lost_match', actorId: req.user.id, extraBody: result.found ? `Potential match: ${result.found.itemName}. Sign in to review the match and contact Campus Lost & Found.` : '' }) : null;
    res.json({ match: result.match, emailDelivery });
  } catch (error) { res.status(404).json({ message: error.message }); }
});
...
app.post('/api/admin/communications/send', auth, requirePerm('templates:manage'), async (req, res) => {
  ...
  for (const channel of channels) { if (channel !== 'in_app' && recipient.preferences?.[channel] === false) { deliveries.push({ channel, status: 'opted_out', detail: `${recipient.name} has disabled ${channel} notifications.` }); continue; } if (channel === 'in_app') deliveries.push({ channel, status: 'sent' }); else if (channel === 'email') deliveries.push({ channel, ...(await sendEmailMessage({ to: recipient.email, subject, body, actionPath: entityType === 'issue' ? `/app/issues/${entity.id}` : `/app/lost-found/${entity.id}` })) }); else if (channel === 'whatsapp') { const phone = normalizePhone(recipient.phone); if (!phone) deliveries.push({ channel, status: 'unavailable', detail: 'The user does not have a valid phone number.' }); else { const to = phone.replace(/^\+/, ''); if (!isVonageAllowedRecipient(db, to)) deliveries.push({ channel, status: 'blocked', detail: `+${to} is not allow-listed in the Vonage Sandbox.` }); else { const vclient = await vonageClient(); if (!vclient || !process.env.VONAGE_WHATSAPP_FROM) deliveries.push({ channel, status: 'not_configured', detail: 'Vonage WhatsApp credentials are missing (VONAGE_APPLICATION_ID / VONAGE_PRIVATE_KEY_PATH / VONAGE_WHATSAPP_FROM).' }); else { try { const result = await vclient.messages.send({ to, from: process.env.VONAGE_WHATSAPP_FROM, channel: Channels.WHATSAPP, messageType: 'text', text: body }); deliveries.push({ channel, status: result.messageUUID ? 'submitted' : 'failed', providerId: result.messageUUID }); } catch (error) { deliveries.push({ channel, status: 'failed', detail: String(error.message || '').slice(0, 300) }); } } } } } else { const phone = normalizePhone(recipient.phone); deliveries.push(phone ? { channel, ...(await sendTwilioMessage({ channel, to: phone, body })) } : { channel, status: 'unavailable', detail: 'The user does not have a valid phone number.' }); } }
...
const createLocalBackup = async (createdBy = 'SYSTEM') => { const db = await readDb(); await fs.mkdir(path.join(root, 'backups'), { recursive: true }); const id = `BKP-${new Date().toISOString().replace(/[:.]/g, '-')}`; const filename = `${id}.json`; const snapshot = { ...db, sessions: [], passwordResets: [], authChallenges: [], backups: [] }; const serialized = JSON.stringify(snapshot, null, 2); await fs.writeFile(path.join(root, 'backups', filename), serialized); return mutateDb((latest) => { const item = { id, filename, createdBy, size: serialized.length, createdAt: now() }; latest.backups.unshift(item); addAudit(latest, createdBy === 'SYSTEM' ? 'backup.scheduled' : 'backup.created', createdBy, { backupId: id }); return item; }); };
app.post('/api/admin/backups', auth, adminOnly, async (req, res) => { const backup = await createLocalBackup(req.user.id); res.status(201).json({ backup }); });
app.get('/api/admin/health', auth, adminOnly, async (_req, res) => { const db = await readDb(); const smtpReady = Boolean(process.env.SMTP_HOST && process.env.SMTP_FROM); const emailDeliveries = db.communications.flatMap((record) => record.deliveries || []).filter((delivery) => delivery.channel === 'email').slice(0, 100); const sentEmails = emailDeliveries.filter((delivery) => delivery.status === 'sent').length; const failedEmails = emailDeliveries.filter((delivery) => delivery.status === 'failed').length; res.json({ status: 'healthy', checkedAt: now(), components: [{ name: 'JSON database', status: 'operational', detail: `${db.users.length} users, ${db.issues.length} issues` }, { name: 'AI suggestion engine', status: 'operational', detail: 'Local heuristic prototype' }, { name: 'Notification queue', status: 'operational', detail: `${db.notifications.length} in-app notifications` }, { name: 'Email delivery', status: smtpReady && !failedEmails ? 'operational' : smtpReady ? 'degraded' : 'not_configured', detail: smtpReady ? `SMTP configured · ${sentEmails} sent · ${failedEmails} failed in recent records` : 'SMTP_HOST and SMTP_FROM are required' }], jobs: db.jobs }); });
app.post('/api/admin/issues/bulk', auth, adminOnly, async (req, res) => {
  const ids = Array.isArray(req.body.ids) ? req.body.ids : [];
  if (!ids.length || ids.length > 100) return res.status(400).json({ message: 'Choose between 1 and 100 issues.' });
  const allowed = ['triaged', 'assigned', 'in_progress', 'resolved', 'closed'];
  if (req.body.status && !allowed.includes(req.body.status)) return res.status(400).json({ message: 'Invalid bulk status.' });
  const result = await mutateDb((db) => {
    const items = db.issues.filter((item) => ids.includes(item.id)); const emailIds = [];
    items.forEach((item) => { const previousStatus = item.status; if (req.body.status) item.status = req.body.status; if (req.body.departmentId) item.departmentId = req.body.departmentId; item.version = (item.version || 1) + 1; item.updatedAt = now(); item.statusHistory.push({ status: item.status, actorId: req.user.id, at: item.updatedAt, reason: 'Bulk operation' }); if (previousStatus !== item.status && ['resolved', 'closed'].includes(item.status)) emailIds.push(item.id); });
    addAudit(db, 'issues.bulk_updated', req.user.id, { ids, status: req.body.status, departmentId: req.body.departmentId });
    return { updated: items.length, emailIds };
  });
  const event = req.body.status === 'resolved' ? 'issue_resolved' : req.body.status === 'closed' ? 'issue_closed' : null;
  const emailDeliveries = event ? (await Promise.allSettled(result.emailIds.map((entityId) => sendAutomaticEventEmail({ entityType: 'issue', entityId, event, actorId: req.user.id })))).map((delivery) => delivery.status === 'fulfilled' ? delivery.value : { status: 'failed', detail: delivery.reason?.message || 'Automatic email failed.' }) : [];
  res.json({ updated: result.updated, emailDeliveries });
});
...
app.post('/api/send/whatsapp', auth, rateLimit(60000, 10), async (req, res) => {
  const to = vonageE164(String(req.body.to || '').trim()); const message = String(req.body.message || '').trim();
  ...
  if (message.length > VONAGE_TEXT_MAX) return res.status(400).json({ message: `Message exceeds the ${VONAGE_TEXT_MAX} character limit for WhatsApp text messages.` });
  const db = await readDb();
  if (!isVonageAllowedRecipient(db, to)) return res.status(403).json({ message: `Recipient +${to} is not allow-listed in the Vonage Sandbox. Add it on the Vonage Dashboard (Messages > Sandbox) and set VONAGE_ALLOWED_RECIPIENTS.` });
  if (!vonageWindowOpen(db, to)) return res.status(403).json({ message: `Recipient +${to} has not messaged the Vonage Sandbox within the last 24 hours. Ask them to send a message to the Sandbox WhatsApp number once (this opens the 24-hour window for free-form messages).` });
  const from = process.env.VONAGE_WHATSAPP_FROM; if (!from) return res.status(503).json({ message: 'Vonage WhatsApp delivery is not configured (VONAGE_APPLICATION_ID / VONAGE_PRIVATE_KEY_PATH / VONAGE_WHATSAPP_FROM).' });
  const client = await vonageClient(); if (!client) return res.status(503).json({ message: 'Vonage delivery is not configured (VONAGE_APPLICATION_ID and VONAGE_PRIVATE_KEY_PATH must be set).' });
  try { const result = await client.messages.send({ to, from, channel: Channels.WHATSAPP, messageType: 'text', text: message }); const messageId = result.messageUUID; const record = await mutateDb((db) => { const rec = { id: makeId('VMSG'), messageUuid: messageId, channel: 'whatsapp', direction: 'outbound', to, from, text: message, status: 'submitted', timestamp: now(), createdAt: now(), sentBy: req.user.id }; db.vonageMessages.push(rec); addAudit(db, 'vonage.whatsapp_sent', req.user.id, { messageUuid: messageId, to }); return rec; }); res.status(201).json({ success: true, messageId, channel: 'whatsapp', status: 'submitted', record: { id: record.id, status: record.status } }); } catch (error) { const detail = String(error?.message || '').slice(0, 300); const status = detail.includes('429') || error?.status === 429 ? 429 : 502; res.status(status).json({ success: false, message: `Vonage send failed (${error?.status || 'unknown'}).`, detail }); }
});
...
app.post('/api/vonage/inbound', vonageSignature, async (req, res) => {
  const body = req.body || {}; const messageUuid = String(body.message_uuid || '').trim(); const from = String(body.from || '').trim(); const to = String(body.to || '').trim(); const text = String(body.text || '').trim(); const channel = String(body.channel || 'whatsapp').trim();
  if (!messageUuid) return res.status(400).json({ message: 'message_uuid is required.' });
  await mutateDb((db) => { const existing = db.vonageMessages.find((m) => m.messageUuid === messageUuid); if (existing) return { duplicate: true }; db.vonageMessages.push({ id: makeId('VMSG'), messageUuid, channel, direction: 'inbound', from, to, text, timestamp: String(body.timestamp || now()), createdAt: now() }); addAudit(db, `vonage.${channel}_received`, 'SYSTEM', { messageUuid }); return { duplicate: false }; });
  res.json({ status: 'received' });
});

app.post('/api/vonage/status', vonageSignature, async (req, res) => {
  const body = req.body || {}; const messageUuid = String(body.message_uuid || '').trim(); if (!messageUuid) return res.status(400).json({ message: 'message_uuid is required.' });
  const status = String(body.status || '').trim(); const errorCode = body.error_code !== undefined && body.error_code !== '' ? String(body.error_code) : null; const errorReason = String(body.error_reason || '').trim() || null;
  await mutateDb((db) => { const record = db.vonageMessages.find((m) => m.messageUuid === messageUuid); if (!record) { db.vonageMessages.push({ id: makeId('VMSG'), messageUuid, channel: String(body.channel || 'whatsapp').trim(), direction: 'status-callback', from: String(body.from || '').trim(), to: String(body.to || '').trim(), status, errorCode, errorReason, timestamp: String(body.timestamp || now()), createdAt: now(), updatedAt: now() }); return { created: true }; } record.status = status; record.errorCode = errorCode; record.errorReason = errorReason; record.timestamp = String(body.timestamp || record.timestamp); record.updatedAt = now(); return { updated: true }; });
  res.json({ ok: true });
});
...
await registerAdvancedFeatures(app, { auth, adminOnly, readDb, mutateDb, makeId, now, notify, addAudit, staffRole, cleanAttachments, sendEmailMessage, jwt, jwtSecret: JWT_SECRET, root });
...
await ensureAdmin();
const autoBackupHours = Number(process.env.AUTO_BACKUP_HOURS || 0);
if (process.env.NODE_ENV !== 'test' && Number.isFinite(autoBackupHours) && autoBackupHours > 0) { const timer = setInterval(() => createLocalBackup().catch((error) => console.error('Scheduled backup failed:', error.message)), Math.max(1, autoBackupHours) * 3600000); timer.unref(); }
